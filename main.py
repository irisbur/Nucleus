...
def main():
    file = open("Samp4.bin", 'rb')
    # content = ''.join(f"{n:08b}" for n in file.read())
    content = file.read()

    compressed_file = compress(content)
    compressed_content = ''.join(f"{n:08b}" for n in compressed_file)
    print(len(compressed_content) / (1024 * 8))

    decompressed_file = decompress(compressed_file)

    # print(compressed_file)
    print(len(decompressed_file))
    print(decompressed_file == content)
